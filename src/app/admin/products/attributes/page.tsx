import {
  AttributesTable,
  SkeletonAttributesTable,
} from '@/components/attributes/table';
import { Heading } from '@/components/ui/heading';
import { Suspense } from 'react';
import { getPaginatedAttributes } from '@/db/drizzle/queries/attributes';
...
async function SuspendedPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const result = await getPaginatedAttributes(
    pageNum,
    perPage,
    sortKey,
    sortDir,
  ...
  return (
    <div className="m-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[30%_1fr] gap-4 mt-4">
        ...
        <div className="">
          <DataTableSearchClient />
          <Suspense fallback={<SkeletonAttributesTable />}>
            <AttributesTable
              attributes={data}
              total={total}
              page={page}
