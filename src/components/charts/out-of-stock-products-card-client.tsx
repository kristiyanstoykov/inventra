...
export function OutOfStockProductsCardClient({
  items,
}: {
  ...
  return (
    <Card className="min-w-[350px] max-w-[350px] flex flex-col shadow-red-500">
      <CardHeader>
        <CardTitle className="flex flex-row items-baseline gap-1">
          {/* Status dot */}
          <span aria-hidden className="h-2.5 w-2.5 rounded-full bg-red-500 mt-1 shrink-0" />
          Out of Stock Products
        </CardTitle>
        <CardDescription>
          {items.length > 0
            ? items.length === 1
              ? `${items.length} product needs restocking`
      ...
      <CardFooter className="flex justify-between text-xs text-muted-foreground">
        ...
        <Link href="/admin/products" className="hover:underline text-red-500 font-medium">
          Manage Products →
        </Link>
      </CardFooter>
    </Card>
